...
const initialState = {
  ...
  price: 100,
  currency: "₹",
  coordinates: { long: "0.000000", lat: "0.000000" },
  additionalImages: [],
};
...
export default function AddListingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.title || !form.location || !form.city || !form.description) {
      showSnackbar("Fill all required fields", "error");
      setLoading(false);
      return;
    }
    // Enforce exactly 5 images
    if (Images.length !== 5) {
      showSnackbar("Please select exactly 5 images.", "error");
      setLoading(false);
      return;
    ...
    try {
      ...
      showSnackbar("Listing added successfully!", "success");
